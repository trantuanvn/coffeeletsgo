...
';
	...
	function print_options_switch($switchValue, $arrayValue) {
		...
		switch ($switchValue) {
			...
			break;
			case 'date_order':
				$this->print_date_order($arrayValue);
			break;
			case 'text':
			...
			break;
			case 'add_text_datepicker':
				$this->print_add_text_datepicker($arrayValue);
			break;
			case 'sidebar':
			...
			break;
			case 'invitations':
				$this->print_invitations($arrayValue);
			break;
	...
	*	),
	*/	
	function print_invitations($value) {
		...
		if(!isset($_GET['ot-single-entry'])) {
			...
			//select the table values
			$values = $wpdb->get_results("SELECT * FROM $table_name ORDER BY $order DESC");
				...
				<tbody>
					<tr class="item">
						<td colspan="2" class="table">
							<div>
								<p class="label-wide"><b><?php echo $value['title'];?></b></p>
							</div>
							<table>
								<tbody>
									<tr class="title">
										<td><?php _e("Nr.", THEME_NAME);?></td>
										<td><?php _e("Name", THEME_NAME);?></td>
										<td><?php _e("Added", THEME_NAME);?></td>
										<td><?php _e("Reservation Date", THEME_NAME);?></td>
										<td><?php _e("E-mail", THEME_NAME);?></td>
										<td><?php _e("Option", THEME_NAME);?></td>
									</tr>
									<?php 
										$i=1;
										foreach($values as $value) {

											$reservationCreated = $value->reservationCreated + $timezone_offet * 3600;
											$reservationDate = $value->reservationDate;
									?>

									<tr<?php if($value->approve=="") { ?> class="unread"<?php } ?><?php if($value->approve=="yes") { ?> class="i-accepted"<?php } ?><?php if($value->approve=="no") { ?> class="i-denied"<?php } ?>>
										<td><?php echo $i;?>.</td>
										<td><?php echo stripslashes($value->name);?></td>
										<td><?php echo date("Y-m-d",$reservationCreated);?><br><?php echo date("H:i",$reservationCreated);?></td>
										<td><?php echo date("Y-m-d",$reservationDate);?><br><?php echo date("H:i",$reservationDate);?></td>
										<td><?php echo $value->email;?></td>
										<td>
											<a href="admin.php?page=theme-configuration&action=view&ot-single-entry=<?php echo $value->id;?>">
												<img src="<?php echo THEME_IMAGE_CPANEL_URL;?>btn-check-1.png" alt="<?php _e("View Single", THEME_NAME);?>" title="<?php _e("View Single", THEME_NAME);?>">
											</a> 
											<a href="admin.php?page=theme-configuration&action=delete&ot-single-entry=<?php echo $value->id;?>">
												<img src="<?php echo THEME_IMAGE_CPANEL_URL;?>btn-cross-1.png" alt="<?php _e("Delete", THEME_NAME);?>" title="<?php _e("Delete", THEME_NAME);?>">
											</a>
										</td>
									</tr>
									<?php
		...
		<?php
			...
			//select the table values
			$values = $wpdb->get_results("SELECT * FROM $table_name WHERE id=$id");

			$timezone_offet = get_option( 'gmt_offset' );
			$reservationCreated = $values[0]->reservationCreated + $timezone_offet * 3600;
			$reservationDate = $values[0]->reservationDate;
		...
		?>
				<div class="item-z">
					<span style="float:right;">
						<a href="admin.php?page=theme-configuration" style=" text-decoration: none;">
							<img src="<?php echo THEME_IMAGE_CPANEL_URL;?>btn-back-1.png" alt="<?php _e("Back", THEME_NAME);?>" title="<?php _e("Back", THEME_NAME);?>">
						</a>
					</span>
					<p class="label"><b><?php _e("Full Entry", THEME_NAME);?></b></p>
				</div>
				<div class="item-z">
					<p class="label"><span><strong><?php _e("Reservation Page Name:", THEME_NAME);?></strong></span></p>
					<div class="setting"><?php echo get_the_title($values[0]->page_id);?></div>
				</div>
				<div class="item-z">
					<p class="label"><span><strong><?php _e("Reservation Create Date:", THEME_NAME);?></strong></span></p>
					<div class="setting">
						<strong><?php _e("Time:", THEME_NAME);?></strong> <?php echo date("H:i", $reservationCreated);?><br>
						<strong><?php _e("Date:", THEME_NAME);?></strong> <?php echo date("Y-m-d", $reservationCreated);?>
					</div>
				</div>
				<div class="item-z">
					<p class="label"><span><strong><?php _e("Reservation Date:", THEME_NAME);?></strong></span></p>
					<div class="setting">
						<strong><?php _e("Time:", THEME_NAME);?></strong> <?php echo date("H:i", $reservationDate);?><br>
						<strong><?php _e("Date:", THEME_NAME);?></strong> <?php echo date("Y-m-d", $reservationDate);?>
					</div>
				</div>
				<div class="item-z">
					<p class="label"><span><strong><?php _e("Name:", THEME_NAME);?></strong></span></p>
					<div class="setting"><?php echo $values[0]->name;?></div>
				</div>
				<div class="item-z">
					<p class="label"><span><strong><?php _e("Phone Number:", THEME_NAME);?></strong></span></p>
					<div class="setting"><?php echo stripslashes($values[0]->phone);?></div>
				</div>
				<div class="item-z">
					<p class="label"><span><strong><?php _e("Mail:", THEME_NAME);?></strong></span></p>
					<div class="setting"><?php echo $values[0]->email;?></div>
				</div>
				<div class="item-z">
					<p class="label"><span><strong><?php _e("People Count:", THEME_NAME);?></strong></span></p>
					<div class="setting"><?php echo stripslashes($values[0]->guests);?></div>
				</div>
				<div class="item-z">
					<p class="label"><span><strong><?php _e("Notes:", THEME_NAME);?></strong></span></p>
					<div class="setting"><?php echo stripslashes($values[0]->notes);?></div>
				</div>
				...
				<div class="item-z">
					<p class="label"><b><?php _e("Send a answer email", THEME_NAME);?></b></p>
				</div>
				<div class="item-z">
				
					<div class="item-z">
						<p><?php _e("To approve or deny the reservation, for the form below and press \"Send & Approve\". The Customer will recieve that message in his e-mail.", THEME_NAME);?></p>
						<p><?php _e("Orange Themes is not responsible if the e-mail goes in the Spam box.", THEME_NAME);?></p>
					</div>
				
					<div class="big-message messsage-color-green" id="ot-mail-success" style="display:none;">
					    <strong><?php _e("Success!", THEME_NAME);?></strong>
					    <span><?php _e("Your message was sent successfully!", THEME_NAME);?></span>
					</div>
					<div id="ot-mail-form">
						<input name="reservation_id" id="ot-reservation-id" type="hidden" value="<?php echo $values[0]->id;?>"/>
						<input name="form_page_id" id="ot-approve-id" type="hidden" value="<?php echo $values[0]->page_id;?>"/>
						<input name="email" id="ot-approve-email" type="hidden" value="<?php echo $values[0]->email;?>"/>
						<div class="item-z">
							<p class="label"><span><strong><?php _e("Type:", THEME_NAME);?></strong></span></p>
							<div class="setting">
								<select name="ot-approve-type" id="ot-approve-type" class="styled">
									<option value="accept"><?php _e("Accept", THEME_NAME);?></option>
									<option value="deny"><?php _e("Deny", THEME_NAME);?></option>
								</select>
							</div>
						</div>
						<div class="item-z">
							<p class="label"><span><strong><?php _e("Text:", THEME_NAME);?></strong></span></p>
							<div class="setting">
								<textarea name="message" class="text-area-1" id="ot-approve-message"></textarea>
								<a href="javascript:void(0);" class="button-1" id="ot-approve-message-1-send"><?php _e("Send Message", THEME_NAME);?></a>
							</div>
						</div>
					</div>
				</div>
	...
	*/
	function print_sidebar_order( $value ) {
		...
		<ul class="blocks block-active clearfix" id="sidebar_order">
			<input type="hidden" id="the-block-id"  value="<?php echo $value["id"];?>"/>
			<?php
			$i=0;
			foreach ( $saved_value as $sidebar ) {
				if ( $sidebar != "" ) {
					$i++;			
			?>
					<li class="row-item-full-width clearfix recordsArray_<?php echo convert_to_class($sidebar); ?>" id="recordsArray_<?php echo convert_to_class($sidebar); ?>" alt="<?php echo $sidebar; ?>">
						<div class="blocks-content clearfix" style="text-align: left;">
							<span class="block-name"><?php _e("Sidebar name:", THEME_NAME);?></span> <b><?php echo $sidebar; ?></b><a href="javascript:{}" class="button edit sidebar-edit" id="edit-<?php echo convert_to_class($sidebar); ?>" rel="<?php echo $sidebar; ?>">Edit</a><a href="javascript:{}" class="button delete sidebar-delete" id="delete-<?php echo convert_to_class($sidebar); ?>">Delete</a>
						</div>
					</li>
			<?php
				}	
		...
		<ul class="blocks block-active clearfix" id="date_order">
			...
			foreach ( $saved_value as $sidebar ) {
				if ( $sidebar != "" ) {
					$i++;			
			?>
					<li class="row-item-full-width clearfix recordsArray_<?php echo convert_to_class($sidebar); ?>" id="recordsArray_<?php echo str_replace("-", "+",convert_to_class($sidebar)); ?>" alt="<?php echo $sidebar; ?>">
						<div class="blocks-content clearfix" style="text-align: left;">
							<span class="block-name"><?php _e("Free Date:", THEME_NAME);?></span> <b><?php echo $sidebar; ?></b>
							<a href="javascript:{}" class="button edit sidebar-edit" id="edit-<?php echo convert_to_class($sidebar); ?>" rel="<?php echo $sidebar; ?>"><?php _e("Edit", THEME_NAME);?></a>
							<a href="javascript:{}" class="button delete sidebar-delete" id="delete-<?php echo convert_to_class($sidebar); ?>"><?php _e("Delete", THEME_NAME);?></a>
						</div>
					</li>
					<script>
						jQuery(document).on("click", ".sidebar-edit", function(){
						    jQuery("#input-<?php echo convert_to_class($sidebar); ?>").datepicker({ 
						     	dateFormat: 'dd-mm-yy',
						        firstDay: 1
						    });
						});
	...
	*		"title" => "Add New Date:",
	*		"id" => THEME_NAME."_add_text_datepicker"
	*		)
	*	)
	*/
	function print_add_text_datepicker($value) {
		if(isset($value['std'])) {
			$default = $value['std'];
		} else {
			$default = false;
		...
		<?php } ?>
			<label><?php echo $value['title'];?></label>
			<span class="input-text"><input type="text" name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" /></span>
		<?php
		}
		$saved_value = get_option( $value['id'].'s' );
		$saved_value = stripslashes($saved_value);
