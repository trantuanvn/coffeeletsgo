 ...
 * -------------------------------------------------------------------------*/
	...
	function save_sidebar(new_name,old_name,old_name_id,new_name_id,title,blockId) {
		var adminUrl = scripts.adminUrl;
		...
		jQuery.ajax({
			url: adminUrl,
			type:"POST",
			data:"action=edit_sidebar&sidebar_name="+new_name+"&old_name="+old_name+"&blockID="+blockId,
			success:function(results) {
				jQuery("#save-"+old_name_id).parent(".edit-wrapper").removeClass("edit-wrapper").addClass("blocks-content");

				jQuery(".recordsArray_"+old_name_id).html("<div class=\"blocks-content clearfix\" style=\"text-align: left;\"><span class=\"block-name\">"+title+"</span> <b>"+new_name+"</b> <a href=\"javascript:{}\" class=\"button edit sidebar-edit\" id=\"edit-"+new_name_id+"\" rel=\""+new_name+"\">Edit</a><a href=\"javascript:{}\" class=\"button delete sidebar-delete\" id=\"delete-"+new_name_id+"\">Delete</a></div>");
				new_name_id_2 = new_name_id.replace(/-/g, '+');
				jQuery(".recordsArray_"+old_name_id).attr("id", "recordsArray_"+new_name_id_2);
				jQuery(".recordsArray_"+old_name_id).removeClass("recordsArray_"+old_name_id).addClass("recordsArray_"+new_name_id);

				var text = results;
				jQuery("#"+blockId+"s").val(text);

				jQuery(document).ready(function($){
					var adminUrl = scripts.adminUrl;
					var themeName = scripts.themeName;
					jQuery(".sidebar-delete").click(function() {
						var blockId = jQuery(this).parent().parent().parent().find('input#the-block-id').val();
						var id = $(this).attr("id");
						id = id.replace('delete-', '');
						jQuery.ajax({
							url:adminUrl,
							type:"POST",
							data:"action=delete_sidebar&sidebar_name="+id+"&blockID="+blockId,
							success:function(results) {
								//alert(results);
...
function OT_sidebar_edit() {
	jQuery(".sidebar-edit").click(function() {
		...
		var title = jQuery(this).parent('.blocks-content').find(".block-name").html();
		var blockId = jQuery(this).parent().parent().parent().find('input#the-block-id').val();

		jQuery(this).parent(".blocks-content").removeClass("blocks-content").addClass("edit-wrapper");
		jQuery(this).parent(".edit-wrapper").html("<span class=\"block-name\">"+title+"</span><input type=\"text\" id=\"input-"+old_name_id+"\" name=\"sidebar-name\" value=\""+old_name+"\" class=\"text\" /><a href=\"javascript:void(0)\" class=\"button edit\" id=\"save-"+old_name_id+"\">Save</a><a href=\"javascript:void(0)\" class=\"button delete sidebar-delete\" id=\"delete-"+old_name_id+"\">Delete</a>")
		...
		if(e.keyCode == 13){
			...
			new_name_id=new_name_id.toLowerCase();
				...
				save_sidebar(new_name,old_name,old_name_id,new_name_id, title,blockId);
			}
		});
	...
	jQuery(document).ready(function($){
		...
		var themeName = scripts.themeName;
		jQuery(function() {
			jQuery("ul#sidebar_order, ul#date_order").sortable({ opacity: 0.6, cursor: 'move', update: function() {
				var blockId = $(this).parent().parent().find('input#the-block-id').val();
				var order = $(this).sortable("serialize") + '&action=update_sidebar&blockID='+blockId;	
				
				jQuery.post(adminUrl, order, function(theResponse){
					console.log(theResponse);
					var text = theResponse;
					jQuery("#"+blockId+"s").val(text);
				});
			}
			});
 ...
 * -------------------------------------------------------------------------*/
	...
	jQuery(document).ready(function($){
		var adminUrl = scripts.adminUrl;
		jQuery("#ot-approve-message-1-send").click(function() {
			var val = jQuery(this).parent().find("textarea#ot-approve-message").val();
			var mail = jQuery(this).parent().parent().parent().find("input#ot-approve-email").val();
			var id = jQuery(this).parent().parent().parent().find("input#ot-approve-id").val();
			var type = jQuery(this).parent().parent().parent().find("select#ot-approve-type").val();
			var reservation_id = jQuery(this).parent().parent().parent().find("input#ot-reservation-id").val();

			jQuery.ajax({
				url:adminUrl,
				type:"POST",
				data:"action=ot_send_email_admin&message="+val+"&mail="+mail+"&type="+type+"&reservation_id="+reservation_id+"&form_page_id="+id,
				success:function(results) {
					console.log(results);
					jQuery('#ot-mail-form').css("display", "none");
					jQuery('#ot-mail-success').css({ 'display': 'block'});
