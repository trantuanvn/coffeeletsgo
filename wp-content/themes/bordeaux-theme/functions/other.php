 ...
 * -------------------------------------------------------------------------*/	
	...
	$my_meta->addSelect(THEME_NAME.'_sidebar_select', $sidebars ,array('name'=> __('Sidebar','tax-meta'), 'std'=> array('default')));
	...
	$my_meta->addSelect(THEME_NAME.'_blog_style',array('1'=>__('Small Images', THEME_NAME),'2'=> __('Large Images', THEME_NAME)),array('name'=> __('Category Style ','category'), 'std'=> array('1')));
	$my_meta->Finish();
...
$orangethemes_buttons=array("orangethemesbutton", "orangethemesspacer", "orangethemesquote", "|",
			 "orangethemesingredients", "orangethemesdirections", "|",
   ...
   ?>
	...
	<li <?php comment_class($comentClass); ?> id="li-comment-<?php comment_ID(); ?>">
		<div class="comment-block" id="comment-<?php comment_ID() ?>">
			<?php if(get_avatar_url(get_avatar( $comment, 50))) { ?>
				<a href="<?php if(get_comment_author_url()) { echo get_comment_author_url();} else { echo "#"; } ?>" class="user-avatar">
					<img src="<?php echo get_avatar_url(get_avatar( $comment, 50));?>" alt="<?php echo get_comment_author();?>" title="<?php echo get_comment_author();?>" />
				</a>
			<?php } ?>
			<div class="comment-text">
				<span class="time-stamp right"><?php printf(__(' %1$s, %2$s', THEME_NAME), get_comment_date("j F, Y"), get_comment_time("H:i"));?></span>
				<strong class="user-nick">
					<a href="<?php if(get_comment_author_url()) { echo get_comment_author_url();} else { echo "#"; } ?>"><?php echo get_comment_author();?></a>
				</strong>
				<?php comment_text(); ?>
				<?php comment_reply_link(array_merge( $args, array('depth' => $depth, 'max_depth' => $args['max_depth'], 'reply_text' => '<i class="fa fa-reply left"></i>'.( __( 'Reply to this comment' , THEME_NAME ))))) ?>
			</div>
...
function orangethemes_guestbook($comment, $args, $depth) {
	$GLOBALS['comment'] = $comment;
	if(!get_avatar_url(get_avatar( $comment, 60))) {
		$comentClass = "noavatar";
	} else {
